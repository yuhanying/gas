...
class ResNet2D(nn.Module):
    def __init__(self, block, num_blocks, num_classes=4):
        ...
        self.in_channels = 64
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1)  # 输入通道从3改为7，即7个GADF通道
    ...
    def forward(self, x):
        ...
        out = self.fc(out)
        y_pred_prob = self.softmax(out)
        return out, y_pred_prob


def ResNet2D18():
    return ResNet2D(BasicBlock2D, [2, 2])

model_ft=ResNet2D18().to(config.device)
...
class EarlyStopper:
    ...
    def early_stop(self, validation_loss,model):
        if validation_loss < self.min_validation_loss:
            ...
            self.counter = 0
            torch.save(model.state_dict(), 'models/res_wo_time.pth')
...
# del combined_model
combined_model =  ResNet2D18().to(config.device)
ckpt = torch.load('models/res_wo_time.pth', map_location='cpu')  # Load your best model
