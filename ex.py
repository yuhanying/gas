...
class EarlyStopper:
    ...
    def early_stop(self, validation_loss,model):
        if validation_loss < self.min_validation_loss:
            ...
            self.counter = 0
            torch.save(model.state_dict(), 'models/img_ensemble_two.pth')
...
# ======multimodal=====
optimizer = optim.SGD(
                      combined_model.parameters(), lr=0.0005, momentum=0.9)
...
# del combined_model
combined_model =  CombinedFCModel().to(config.device)
ckpt = torch.load('models/img_ensemble_two.pth', map_location='cpu')  # Load your best model
