...
class sensordata(Dataset):  # inheriting Dataset, not nn.Module
    def __init__(self, target, MQ=None, image=None, transform=None, seq_len=1):
...
class SimplifiedVGG19_2D(nn.Module):
    def __init__(self, num_classes):
        super(SimplifiedVGG19_2D, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=3, padding=1),  # 输入通道从2改为7，即7个GADF通道
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
        )
        self.avgpool = nn.AdaptiveAvgPool2d((4, 4))  # 更改为2维的自适应平均池化
...
model_ft=SimplifiedVGG19_2D(4).to(config.device)
...
class EarlyStopper:
    ...
    def early_stop(self, validation_loss,model):
        if validation_loss < self.min_validation_loss:
            ...
            self.counter = 0
            torch.save(model.state_dict(), 'models/cnn_wo_time.pth')
...
# del combined_model
combined_model =  SimplifiedVGG19_2D(4).to(config.device)
ckpt = torch.load('models/cnn_wo_time.pth', map_location='cpu')  # Load your best model
